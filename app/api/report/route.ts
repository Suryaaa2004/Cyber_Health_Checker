  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.log('[API] Generating mock PDF report for preview, size:', mockPdfBuffer.length);

        return new NextResponse(Buffer.from(mockPdfBuffer), {
          headers: {
            'Content-Type': 'application/pdf',
            'Content-Disposition': `attachment; filename="${body.domain}-security-report.pdf"`,
          },
        ...
        console.error('[API] Mock PDF generation also failed:', pdfError);
        throw pdfError;
      }
    }
  } catch (error) {
    ...
    console.error('[API] Detailed error:', { errorMessage, errorType: typeof error });
    
    return NextResponse.json(
      { detail: `Report generation failed: ${errorMessage}` },
      { status: 500 }
    );
  }
}
